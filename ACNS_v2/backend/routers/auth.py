"""
routers/auth.py — REST endpoints for authentication and user profiles.

All business logic lives in services/users.py; this router only validates
input, verifies the caller's identity where required, and maps results to
HTTP responses.
...
from models.schemas import LoginRequest, SignupRequest, UserUpdateRequest
...
def login(payload: LoginRequest):
    ...
    """
    try:
        identity = users.identity_from_token(payload.idToken)
        profile = users.ensure_user_profile(
            uid=identity["uid"], email=identity["email"]
        )
        profile = users.record_login(profile["email"], uid=identity["uid"])
        if profile is None:
...
def get_profile(userId: str):
    ...
    profile = users.get_user_profile(userId)
    if not profile:
...
def get_profile_by_uid(uid: str):
    ...
    profile = users.get_user_by_uid(uid)
    if not profile:
...
def list_users(
    ...
    return {"success": True, "users": users.list_users(include_inactive=includeInactive)}
...
def update_user(userId: str, payload: UserUpdateRequest):
    ...
    try:
        ...
        profile = users.update_user_profile(
...
def deactivate_user(userId: str):
    ...
    profile = users.set_user_active(userId, False)
    return {"success": True, "user": profile}
...
def activate_user(userId: str):
    ...
    profile = users.set_user_active(userId, True)
    return {"success": True, "user": profile}
