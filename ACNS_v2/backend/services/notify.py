"""
services/notify.py — Notification orchestrator.
...
(``templates/sms/``): the supervisor's ``preferredLanguage`` is read from the
``users`` profile, the matching template is loaded, and the body is generated.
...
assigned supervisor email (from the issue category map) is looked up in the
``users`` collection and the SMS is sent to ``users/{email}.phoneNumber``.
A missing profile or a missing phone number only logs a warning — the issue
reporting flow continues normally.
"""
...
from config import DEFAULT_CAMPUS_ID, DEFAULT_PREFERRED_LANGUAGE, FRONTEND_BASE_URL
...
def notify_issue_assigned(
    ...
    _dispatch_assignment_sms(issue_id, supervisor_email, category)


def _phone_from_profile(supervisor_email: str, profile) -> str | None:
    """
    Resolve the supervisor's SMS number from their user profile.
    ...
    if not profile:
        logger.warning(
            "No user profile found for supervisor '%s'; SMS skipped.", supervisor_email
        )
        return None
    phone = profile.get("phoneNumber")
    if phone is None:
        logger.warning(
            "Supervisor '%s' has no phoneNumber on file; SMS skipped.", supervisor_email
        )
        return None
    if not isinstance(phone, str):
        logger.warning(
            "Supervisor '%s' phoneNumber is not a string (%s); coercing to string.",
            supervisor_email, type(phone).__name__,
        )
        phone = str(phone)
    phone = phone.strip()
    if not phone:
...
def _dispatch_assignment_sms(
    ...
    profile = get_user_profile(supervisor_email)
    phone = _phone_from_profile(supervisor_email, profile)
    if not phone:
        return
