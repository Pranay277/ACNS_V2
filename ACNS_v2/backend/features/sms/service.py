"""
features/sms/service.py — SMS dispatch abstraction.

The notification service (``features/notifications/service.py``) calls
:func:`send_sms`; the actual delivery is delegated to a provider selected via
``config.SMS_PROVIDER``. Only the placeholder :class:`AndroidGatewayProvider`
exists in this phase, so ``send_sms`` logs the request instead of delivering a
real SMS.
...
Localization lives in dedicated template modules (``features/sms/templates/``),
never here. :func:`send_issue_assigned_sms` resolves a supervisor's preferred
